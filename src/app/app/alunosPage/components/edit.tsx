...
export function Edite({user}: {user:Aluno}){
  ...
  const update =  form.handleSubmit(async (data) =>{
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/user/update/${user.id}`,{
      method:'PUT',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
     ...
     <div className="flex max-sm:flex-col"> 
      <div className="w-2/6 max-sm:w-full first-letter:h-full ">
        ...
        <div className="space-y-4 flex  flex-col justify-stretch">
          <div className="flex flex-col justify-stretch">
            ...
            <input className="px-3 py-3 text-zinc-300 bg-bg rounded-lg " defaultValue={`${user.name}`} type="text" required  {...form.register('name')} />
          </div>
          <div className="flex flex-col justify-stretch" >
            <h1 className="text-zinc-300 font-bold">Email:</h1>
            <input className="px-3 py-3 text-zinc-300 bg-bg rounded-lg " defaultValue={`${user.email}`} type="text" required {...form.register('email')} />
          </div>
      ...
      <div className="ml-4 w-3/6 max-sm:w-full max-sm:ml-0 flex flex-col space-y-4">
        <div className="flex flex-col justify-stretch" >
            ...
            <input className="px-3 py-3 text-zinc-300 bg-bg rounded-lg " defaultValue={`${user.cpf}`} required type="text" {...form.register('cpf')}  />
          </div>
        <div className="flex flex-col justify-stretch" >
            <h1 className="text-zinc-300 font-bold">Tel:</h1>
            <input className="px-3 py-3 text-zinc-300 bg-bg rounded-lg " defaultValue={`${user.tel}`} required type="text" {...form.register('tel')} />
        </div>
        <div className="flex flex-col justify-stretch" >
            <h1 className="text-zinc-300 font-bold">Tel. Emergência:</h1>
            <input className="px-3 py-3 text-zinc-300 bg-bg rounded-lg " defaultValue={`${user.emerg}`} required type="text" {...form.register('emerg')}  />
        </div>
      ...
      <div className="flex flex-col justify-stretch" >
          ...
          <select className="bg-bg  text-zinc-300 rounded-lg p-3" required   defaultValue={`${user.plano}`} id="planos " {...form.register('plano')}>
            {planos?.map(p =>{
        ...
        <div className="flex flex-col justify-stretch" >
            ...
            <input className="px-3 py-3 text-zinc-300 bg-bg rounded-lg " defaultValue={`${user.mensalidade}`} required placeholder="--/--/--" type="date" {...form.register('matricula')} />
        </div>
        <div className="flex flex-col justify-stretch" >
