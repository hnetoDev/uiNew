...
import { Exercicio, Treino } from "@/app/types";
...
export default function EditTreino(treinoR:Treino) {
  ...
  useEffect(() => {
    ...
    const exerciciosRes = JSON.stringify(treinoR.exercicios);
    const exerArr:Array<any> = JSON.parse(exerciciosRes)
    setQtDays(exerArr.length);
    setTreino(exerArr);
    getData()
  }, [])
  ...
  const handlePost = form.handleSubmit(async (dataF) => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/treino/${treinoR.id}`, {
      method: "PATCH",
      body: JSON.stringify({
        exercicios: data.map(d => d.count),
        name: dataF.name,
  ...
  return <div className="flex flex-col  h-max items-center p-6 space-y-4">
    <h1 className=" text-zinc-300 fontC font-bold text-center text-3xl">Edite o treino: {treinoR.name}</h1>
    <div className="flex flex-col w-max h-max bg-zinc-900 space-y-4 rounded-lg p-6 max-sm:flex-col items-center justify-between">
      <div className="flex w-max space-x-6 space-y-4 rounded-lg p-6 max-sm:flex-col items-center justify-between" >
        ...
        <div className="space-y-4">
          <div>
            ...
            <input defaultValue={treinoR.name as string} className="px-3 py-3 focus-within:outline-0 focus-within:rounded-lg focus-within:border focus-within:border-yellow-300  text-zinc-300 bg-bg rounded-lg " {...form.register('name')} type="text" />
          </div>
          <div>
      ...
      <button onClick={handlePost} className=" border border-yellow-400 rounded-lg flex p-2 items-center space-x-3 justify-center" type="submit">
        ...
        <h1 className=" text-yellow-400 font-bold text-lg">Editar Treino</h1>
      </button>
    </div>
              ...
              {treino[item.id - 1].map((t) => {
                ...
                return <div className="mt-6 flex justify-between items-center space-x-2" key={`${integer}`}>
                  ...
                  <div className="flex space-x-3">
                    <input defaultValue={t.series} className="p-3 w-28 text-zinc-300 bg-bg rounded-lg " onChange={(v) => {
                    ...
                    <input defaultValue={t.repeticao} className="p-3 w-36 text-zinc-300 bg-bg rounded-lg " onChange={(v) => {
                      if (Number(v.target.value) < 0) {
                        v.target.value = '0';
                        return
                      }
                    ...
                    <input defaultValue={t.restTime} className="p-3 w-44 text-zinc-300 bg-bg rounded-lg " onChange={(v) => {
                      if (Number(v.target.value) < 0) {
                        v.target.value = '0';
                        return
                      }
